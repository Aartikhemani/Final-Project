...
def show_cart(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'app/add_to_cart.html', {'cart': cart, 'total_amount':total_amount})
...
class ProfileView(View):
    ...
    def post(self,request):
        ...
        if form.is_valid():
            ...
            register = Customer(user=user,name=name,city=city,zipcode=zipcode,state=state,country=country)
            register.save()
...
def address(request):
    ...
    return render(request, 'app/address.html', {'customer_address': customer_address, 'active': 'btn-primary'})
...
def retro_walk(request,data=None):
    if data == None:
        ...
        retro = Product.objects.filter(category__title__contains='Retro Walk').order_by("selling_price")
    elif data == "h2l":
        retro = Product.objects.filter(category__title__contains='Retro Walk').order_by("-selling_price")
...
def bold_daisy(request,data=None):
    if data == None:
        ...
        daisy = Product.objects.filter(category__title__contains='Bold Daisy').order_by("selling_price")
    elif data == "h2l":
        daisy = Product.objects.filter(category__title__contains='Bold Daisy').order_by("-selling_price")
...
def top_wear(request,data=None):
    if data is None:
        ...
        top = Product.objects.filter(category__title__contains='Top Wear').order_by("selling_price")
    elif data == "h2l":
        top = Product.objects.filter(category__title__contains='Top Wear').order_by("-selling_price")
...
def bottom_wear(request,data=None):
    if data is None:
        ...
        bottom = Product.objects.filter(category__title__contains='Bottom Wear').order_by("selling_price")
    elif data == "h2l":
        bottom = Product.objects.filter(category__title__contains='Bottom Wear').order_by("-selling_price")
